...
from .models import UygulamaAyarları, Settings
...
def get_api_key():
    try:
        api_key_ayar = Settings.objects.get(key='google_api')  # 'api_key' alanını kullanarak get()
        return api_key_ayar.value  # 'ayar_degeri' alanını döndürün
    except UygulamaAyarları.DoesNotExist:
        return None

# Google Gemini API anahtarını yapılandırma
api_key = get_api_key()
